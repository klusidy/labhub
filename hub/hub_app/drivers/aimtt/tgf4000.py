...
class TGF4000(Device):
    ...
    def __init__(self, dev_id: str, options: Dict[str, Any]):
        self.PORT = options.get("port", "COM8") # <-- change to your COM port (e.g., "/dev/ttyACM0" on Linux)
        self.BAUD = options.get("baud", 115200) # CDC ignores baud, but pyserial wants a value
        self.ser = None
