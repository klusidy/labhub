...
class KPZ101(KinesisDevice):
    ...
    async def connect(self) -> None:
        ...
        def _connect():
            ...
            dev.StartPolling(self.poll_ms); time.sleep(max(0.25, self.poll_ms/1000))
            dev.EnableDevice();             time.sleep(0.25)

            max_voltage = dev.GetMaxOutputVoltage()  # This is stored as a .NET decimal
            dev.SetMaxOutputVoltage(max_voltage)
            return dev

        self._dev = await self._on_device(_connect) # TODO: better naming than _on_device
