"""
Chameleon SDR — SDRlab 122-16 FPGA driver for labhub.
...
Device hierarchy::
    ...
    chameleon_sdr            ← root Device (handles connection)
    ├── router               ← 16×16 AXI-Stream crossbar
    ├── adc_input            ← 16-bit LTC2145-16 ADC input (no dfilt1)
...
Configuration (config.yaml)::
    ...
    - id: chameleon_sdr_01
      driver: red_pitaya.chameleon_sdr
        ...
        def on_done(t: asyncio.Task):
            try:
                if not t.cancelled() and (exc := t.exception()):
                    logger.error(
                        "%s.scope: raw stream crashed: %s", self._parent_id, exc
                    )
...
@api_device()
class chameleon_sdr(Device):
    """
    Chameleon SDR FPGA board on a SDRlab 122-16.

    Connects via SSH, programs the FPGA, starts the register-access server,
    and exposes all discovered modules as child devices.

    The ADC is a 16-bit LTC2145-16 running at 122.88 MHz (no dfilt1 filter).
    """
    ...
    config_template = {
        ...
        "password": "root",
        "server_port": 2222,
        "fpga_filename": "chameleon_basic_sdr_latest.bit",
        "fpga_dir_src": None,
        "server_dir_src": None,
        "overwrite_server": False,
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            mgr_kwargs = dict(
                ...
                fpga_filename=opt.get("fpga_filename", "chameleon_sdr_latest.bit"),
                overwrite_server=opt.get("overwrite_server", False),
                clock_hz=opt.get("clock_hz", _CLOCK_HZ),
            )
    ...
    class adc_input(ChildDevice):
        """ADC input — 16-bit LTC2145-16 live sample readout.

        Note: filter_bypass register exists in hardware but dfilt1 is not
        instantiated in the SDR design (16-bit data is already s0.15 scale).
        The bypass property is accepted but has no effect.
        """
        ...
        @api_property(doc="Filter bypass (no-op on SDR design — dfilt1 not present)")
    ...
    class asg_0(ChildDevice):
        ...
        @api_property(min=1.0, max=61.44e6, unit="Hz", doc="Output frequency")
        ...
        @api_property(doc="Size of waveform table in samples", max=8192)
    ...
    class scope(ChildDevice):
        ...
        @api_property(
            min=1, max=131071, doc="Decimation factor (1 = 122.88 MSa/s full rate)"
        )
        ...
        @api_property(unit="Hz", doc="Effective sample rate = 122.88 MHz / decimation")
