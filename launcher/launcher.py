...
class Launcher:
    def __init__(
        ...
        self._launcher_kay = f"labhub_tray_{hashlib.sha256(str(self.config_path).encode('utf-8')).hexdigest()[:12]}"
        # Clean up stale socket from previous crash
        try:
            QLocalServer.removeServer(self._launcher_kay)
        except Exception:
            pass
        self._server = QLocalServer(self.app)
        if not self._server.listen(self._launcher_kay):
            sock = QLocalSocket()
            sock.connectToServer(self._launcher_kay)
    ...
    def _build_menu(self):
        self.act_start = QAction("Start server", self.menu)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="127.0.0.1",
        help="Host for the hub server, default is 127.0.0.1 (localhost)",
    )
    parser.add_argument(
        "--port",
        type=int,
